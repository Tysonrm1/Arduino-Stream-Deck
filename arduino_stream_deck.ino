  ...
  if ((millis() - prevTime_F13) > waitTime_F13) {
    if (currRead_F13 != currState_F13) {
      ...
      if (currState_F13 == LOW) {
        // Send the code
        buf[2] = 104;    // HID: F13 key
#ifdef SERIAL_DEBUG
        buf[2] = 'F13';     // Serial: F13 key
#endif
  ...
  if ((millis() - prevTime_F14) > waitTime_F14) {
    if (currRead_F14 != currState_F14) {
      ...
      if (currState_F14 == LOW) {
        // Send the code
        buf[2] = 105;   // HID: F14 key
#ifdef SERIAL_DEBUG
        buf[2] = 'f14';   // Serial: F14 key
#endif
        Serial.write(buf, 8); // Send keypress
      } else {
        // Release the keyboard
  ...
  if ((millis() - prevTime_F15) > waitTime_F15) {
    if (currRead_F15 != currState_F15) {
      ...
      if (currState_F15 == LOW) {
        // Send the code
        buf[2] = 106;    // HID: F15 key
#ifdef SERIAL_DEBUG
        buf[2] = 'f15';     // Serial: F15 key
#endif
        Serial.write(buf, 8); // Send keypress
      } else {
        // Release the keyboard
  ...
  if ((millis() - prevTime_F16) > waitTime_F16) {
    if (currRead_F16 != currState_F16) {
      ...
      if (currState_F16 == LOW) {
        // Send the code
        buf[2] = 107;   // HID: F16 key
#ifdef SERIAL_DEBUG        
        buf[2] = 'f16';   // Serial: F16 key
#endif
        Serial.write(buf, 8); // Send keypress
      } else {
        // Release the keyboard
...
if ((millis() - prevTime_F18) > waitTime_F18) {
    if (currRead_F18 != currState_F18) {
      ...
      if (currState_F18 == LOW) {
        // Send the code
        buf[2] = 109;    // HID: F18 key
#ifdef SERIAL_DEBUG
        buf[2] = 'f18';     // Serial: F18 key
#endif
        Serial.write(buf, 8); // Send keypress
      } else {
        // Release the keyboard
  ...
  if ((millis() - prevTime_F21) > waitTime_F21) {
    if (currRead_F21 != currState_F21) {
      ...
      if (currState_F21 == LOW) {
        // send the code
        buf[2] = 112;     // HID F21 key
#ifdef SERIAL_DEBUG
        buf[2] = 'f21';      // Serial F21 key
#endif
        Serial.write(buf, 8); // Send keypress
      } else {
        // Release the keyboard
  ...
  if ((millis() - prevTime_F22) > waitTime_F22) {
    if (currRead_F22 != currState_F22) {
      ...
      if (currState_F22 == LOW) {
        // send the code
        buf[2] = 113;     // HID F22 key
#ifdef SERIAL_DEBUG
        buf[2] = 'f22';      // Serial F22 key
#endif
        Serial.write(buf, 8); // Send keypress
      } else {
        // Release the keyboard
  ...
  if ((millis() - prevTime_F23) > waitTime_F23) {
    if (currRead_F23 != currState_F23) {
      ...
      if (currState_F23 == LOW) {
        // send the code
        buf[2] = 114;     // HID F23 key
#ifdef SERIAL_DEBUG
        buf[2] = 'f23';      // Serial F23 key
#endif
        Serial.write(buf, 8); // Send keypress
      } else {
        // Release the keyboard
  ...
  if ((millis() - prevTime_F24) > waitTime_F24) {
    if (currRead_F24 != currState_F24) {
      ...
      if (currState_F24 == LOW) {
        // send the code
        buf[2] = 115;     // HID F24 key
#ifdef SERIAL_DEBUG
        buf[2] = 'f24';      // Serial F24 key
#endif
        Serial.write(buf, 8); // Send keypress
      } else {
        // Release the keyboard
  ...
  if ((millis() - prevTime_scrollLock) > waitTime_scrollLock) {
    if (currRead_scrollLock != currState_scrollLock) {
      if (currState_scrollLock == HIGH) {
        // send the code
        buf[2] = 71;      // HID ScrollLock key
#ifdef SERIAL_DEBUG
        buf[2] = 'scrollLock';  // Serial ScrollLock key
#endif
        Serial.write(buf, 8); // Send keypress
        delay(100);
        // Release the key
